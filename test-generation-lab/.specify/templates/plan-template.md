...
- Testing by Specification: Plan states TDD intent (RED-GREEN-REFACTOR) and
  confirms tests are derived from specifications before implementation.
- Testing Pyramid + Quality Gates: Test strategy states ~70/20/10
  unit/integration/e2e split and defines how CI enforces >=80% line,
  >=75% branch, and >=75% mutation thresholds.
- Static Analysis Gates: Plan includes strict TypeScript checks (`tsc
  --noEmit`) and lint checks for every protected-branch merge.
